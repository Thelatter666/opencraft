...
> ⚠ **订正（2026-09-16，T-F1 裁决 1）**：初版写「只有三个字段、7 bit 对齐到 u8」。
> 实现证明**放不下**——汇总为 **12 bit**，故实际采用 **2 字节/体素**（惰性分配，
> 无流体区块零开销）。下表已按实际实现补全。详见 §8.2 与 T-F1 裁决 1。

一次流体更新需要的状态量合计 12 bit：

| 字段 | 位宽 | 取值域 | 语义 |
|---|---|---|---|
| 种类（kind） | 2 bit | 枚举（无 / 水 / 岩浆 / 预留） | 决定 ΔL、tick 间隔、搜索半径 |
| 能级 $L$ | 4 bit | 整数 $[0, 8]$ | 8 = 源或下落柱；$1..7$ = 水平流动，越小越浅；0 = 无 |
| 下落中（falling） | 1 bit | 布尔 | 上方有同种流体且向下畅通 |
| **源（source）** | **1 bit** | **布尔** | **不可**由"上方无流体"推导：下落柱可落在已存在的源头上，柱退去时源不能随之蒸发 |
| **发射集（spread）** | **4 bit** | **四方向任意子集** | **坡度寻路产出的发射方向集合**（§4.5）。§3.2 把接收方邻域定义为"能有效流向本格的邻居"，故一格的新能级取决于**哪些邻居朝自己发射**；§4.5 又允许四方向的**任意子集** ⇒ 不是单个枚举值，必须 4 bit |
...
> ⚠ **勘误（2026-09-16，T-F1 裁决 2；只读技术工独立复算确认）**
> 本节的**结论方向正确**（水收窄成一股朝落差走），但**算例数值有误，不得作为数值基准**：
> 落差口 (1,4) 到源 (2,2) 的曼哈顿距离本就是 **3**；S 方向经 (2,3)→(1,3)→(1,4)
> **同样 3 步可达** ⇒ 按 §4.5 的掩码规则应为 **{W, S}**，不是只有 {W}。
> 现有布局造不出"只有 W 侧可达"的效果。**后续卡请自行构造布局，不要照抄本节数值。**
...
- 水平四邻中**至少 2 个**是同种流体的源（**注意：只数水平四邻，不含上下、不含对角**）。
  ⚠ **订正（2026-09-16，只读技术工复核 / PM 裁决 D-3）**：本节漏了源再生的
  **第二条独立规则**，且上面"不含上下"的限定**只适用于第一条**，不是全局规则：

  > **规则二**：流动格若「**1 个**水平相邻源」+「**正上方 1 个源**」，同样晋升为源。
  > （wiki Water 页明确记载；这是独立于"横邻 ≥2 源"的**另一条充分条件**。）

  ⇒ 完整判据应为 `规则一（横邻≥2源）∨ 规则二（横邻1源 ∧ 上方1源）`，再与
  $\mathcal{G}_{\text{water}}$ 和 $\mathcal{B}_{\text{floor}}$ 合取。
  阶段 2 的源再生卡**必须实现两条**，否则 2×2 水池一类原版常识会不一致。
  ⚠ 来源拼写冲突：wiki 同页一处写 `waterSourceConversion`（本节采用）、
  另一处写 `water_source_conversion` ⇒ 本仓统一用**前者**。
...
| **B. 独立流体层（并行位平面）** | 与 `Chunk` 的 `PaletteSection` 平行，加一个 per-voxel 的流体数组（**2 字节/体素**：kind 2 + level 4 + falling 1 + **source 1** + 保留 4 + **spread 4**，见 §1.2 订正） | 每 section 额外 8192 字节（惰性分配 ⇒ **无流体区块零开销**）；序列化格式 v1→v2；网格化要读第二通道 | ✓ 与 §1.1 的 1.13 设计同构，**推荐（已实现，T-F1）** |
