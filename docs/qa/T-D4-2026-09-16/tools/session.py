...
def cmd_focus():
    """Hand the foreground back to another app: a synthetic hold dies when the
    game loses key focus (docs/05 §3.1 rule 2) and, more importantly, the real
    keyboard stops reaching the game - the autosave wait below must not be
    polluted by whoever is typing on this machine."""
    out = subprocess.run(["pgrep", "-x", "Finder"], capture_output=True, text=True).stdout.split()
    if not out:
        out = subprocess.run(["pgrep", "-x", "Terminal"], capture_output=True, text=True).stdout.split()
    if not out:
        raise SystemExit("no Finder/Terminal to hand the focus to")
    subprocess.run([TOOL, "activate", out[0]], check=True)
    print(f"focus handed to pid {out[0]}")
...
def main():
    ...
    if command == "start":
        ...
        cmd_key(int(sys.argv[2]), int(sys.argv[3]))
    elif command == "log":
        cmd_log(*sys.argv[2:])
    elif command == "focus":
        cmd_focus()
