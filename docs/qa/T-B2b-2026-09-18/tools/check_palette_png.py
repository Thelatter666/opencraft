#!/usr/bin/env python3
"""T-B2b · 用 **第三方** PNG 解码器（Pillow）复核 assets/palettes/mossback.png。
...
与 T-B2 原件的差异只有 WANT 表：躯干组色 9..16 → 17..24（机械 +8），
9..16 现按 v2 声明为 U0（关节第二色暂空），一并钉住。
...
# colorIndex -> 期望 sRGB（契约 v2，与 mossback_layers.txt / mossback_palette.txt 的字符表一致）
WANT = {
    ...
    # 关节第二色 9..16：mossback 未用，暂空填 U0（v2 新语义）
