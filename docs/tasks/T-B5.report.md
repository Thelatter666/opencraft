...
|---|---|---|---|
| 1 | 真文件（`.vox` 头 `VOX `、调色板真 PNG `\x89PNG`） | ✓ 解析器读通 | ✓ |
| 2 | 尺寸正好（画布 ≤32、调色板 PNG 16×16） | 7×17×14 / 16×16 | 12×9×18 / 16×16 |
| 4 | alpha：不透明全 255 | ✓ `{255}` | ✓ `{255}` |
| 5 | 色板：每个 RGB 都在 §2.1 的 32 色表内 | ✓ 14 个索引全在表内 | ✓ 15 个索引全在表内 |
| 6 | 低饱和：每色 S ≤ 0.50 | 最高 **0.471**（F2） | 最高 **0.414**（E0） |
| 9 | 原创性：未参考任何既有作品纹理；未用 AI；提示词不适用 | ✓ 逐格手写字符稿 | ✓ 逐格手写字符稿 |
| 10 | 生效：启动日志计数与落盘一致 | ✓ `mobs: 2/3` + 两行 `mob model` | ✓ |
...
基准：Mojang `bedrock-samples` 的 `cow.geo.json`（`geometry.cow.v1.8`，本卡证据目录
`pm_verify/cow.geo.json` 有存档）。1 px = 1/16 格，1 体素 = 0.1 格 = 1.6 px。
