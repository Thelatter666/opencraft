...
苔丘从三层悬檐圆顶压成**一格薄盖**，色板一字未动）；Hollow Wretch 从 169 体素加密到 **496 体素**
（四肢 2 格宽、胸腔凹 2 格深、锯齿破布下摆、眉弓 + 下坠下颌、**2×2 眼窝仍是表面着色**），
...
| `assets/palettes/mossback.png` | **0 字节变化**（md5 相同） | 16×16 RGBA，alpha 全 255 |
| `assets/palettes/hollow_wretch.png` | 130 → 129 字节 | 只有格 13/14：U0 → S3（左右脚骨色） |
...
`tools/{mossback,hollow_wretch}_palette.txt`；工具：`tools/{vox_build,vox_inspect,check_palette_png}.py`。
...
```
[2026-09-18 20:57:27.130] [info] atlas: 60/63 block tiles loaded from ../assets/blocks
[2026-09-18 20:57:27.130] [info] mobs: 2/3 mob models loaded from ../assets/mobs
[2026-09-18 20:57:27.130] [info] mob model mossback: 705 voxels, 1544 triangles, 7 joints, 1.4 blocks tall, palette from palettes/mossback.png
[2026-09-18 20:57:27.130] [info] mob model hollow_wretch: 496 voxels, 1592 triangles, 6 joints, 1.8 blocks tall, palette from palettes/hollow_wretch.png
```
...
**移走 `assets/mobs/` 后** —— `run2_startup_no_mobs_dir.log`：

```
[2026-09-18 20:48:03.820] [info] mobs: 0/3 mob models loaded from ../assets/mobs
```

`0/3` ✓、无 WARN ✓（缺目录仍是**静默回退**，T-B2 立的契约未被这版资产破坏）。

**全量测试**：`459/459`（`ctest_summary.txt`）—— 与 T-B3 基线同数。
...
| 方位 / 仰角 | 从正前方（+Y）往右 **38°** / 俯视 **16°** |
| 投影 | **针孔透视**：屏幕点 = (x_cam / z_cam, y_cam / z_cam) |
| 缩放 | **16 px / 体素**（在 target 距离上），画布固定 760×620 —— **不按模型自适应缩放** |

> 最后一条是本卡特意加的：若出图按模型大小自适应缩放到"长边 520 px"，v4 与 v5 会被缩到**一样大**，
> 比例对照就失去意义。固定 px/体素后，两张图可以并排直接比"谁腿更长、谁身子更厚"。

### 4.2 "这张图确实不是正交投影"的机器自证

地面上取**等长**（1 体素）的两小段，一段在近端、一段在远端，投到屏幕上的像素长度：

| 模型 | 近端 | 远端 | 比值 | 正交相机应为 |
|---|---|---|---|---|
| Mossback v5 | 14.63 px | 8.38 px | **1.745** | 1.000 |
| Hollow Wretch v5 | 12.73 px | 9.52 px | **1.336** | 1.000 |

（工具里是一条 `assert near_px > far_px * 1.02`：不成立就直接中止出图。
旧版两件同参数跑出的比值是 1.580 / 1.213 —— 同样 >1。）
...
| `renders/hollow_wretch_gamecam_ab.png` | v4 `169` 体素 | v5 `496` 体素 | **"不再像棍子"**：四肢有粗细、胸腔有体量、头有骨相（眉弓/下颌/2×2 眼窝） |

单张 + 正交五视图 + 剪影在 `renders/*_v5_gamecam.png`、`*_v5_contact_sheet.png`、`*_v5_silhouette.png`。
...
per-colorIndex bbox and joint mapping:
  ...
  idx  2    62 voxels  x 1..5  y 13..16  z 9..13  rgb (168, 132, 92)  -> joint 1 head 头
...
second-colour indices painted: []
exposed faces: 772  (budget 2000)
triangles    : 1544  (what the startup log reports)
faces per colorIndex: 1:116, 2:108, 3:66, 4:66, 5:66, 6:66, 7:14, 17:56, 18:30, 19:79, 20:68, 21:10, 22:14, 23:8, 24:5
...
gamecam camera: eye=(30.13, 42.59, 19.40) target=(3.50, 8.50, 7.00) dist=45.0 voxels (4.5 blocks) az=38.0 el=16.0 focal=720.0px
perspective self-check: a 1-voxel ground segment projects to 14.63 px at the near end vs 8.38 px at the far end (ratio 1.745; an orthographic camera would give 1.000)
...
palette png : assets/palettes/mossback.png 16x16 depth 8 color type 6 (136 bytes)
...
| 四腿 x 与躯干外缘对齐 | 逐格打印 | `torso x range 0..6; legs x [0, 1, 5, 6]; flush: True` + 上面 24 行逐格清单（左腿 x0..1 / 右腿 x5..6） | ✓ |
| 苔顶 ≤ 头 max z | 不再"苔比头高" | 苔（索引 20-24）max z = **13**；头（索引 2）max z = **13** ⇒ 取**等号**成立 | ✓（见 §9.1 的口径说明） |
| 每索引包围盒表 | 全表贴出 | 上面 `per-colorIndex bbox` 全表（14 个索引） | ✓ |
...
per-colorIndex bbox and joint mapping:
  ...
  idx  2   108 voxels  x 3..8  y 3..8  z 13..17  rgb (226, 230, 236)  -> joint 1 head 头
  ...
  idx  6    44 voxels  x 6..7  y 1..4  z 1..7  rgb (74, 74, 80)  -> joint 5 leg_r 左后肢/右腿
  idx  9     4 voxels  x 5..6  y 2..2  z 11..12  rgb (46, 42, 58)  -> joint 0 body 躯干
  idx 10     8 voxels  x 3..8  y 7..7  z 15..16  rgb (40, 40, 44)  -> joint 1 head 头
...
joint groups actually present (body = index 1 plus every 17+ colour):
  ...
  joint 5 leg_r 左后肢/右腿: 52 voxels
head max z  : 17
...
second-colour indices painted: [9, 10, 11, 12, 13, 14]
  ...
  idx 10 -> joint 1 head 头: cells x 3..8 y 7..7 z 15..16 within joint bbox x 3..8 y 3..8 z 13..17  [OK]
...
eye voxels (idx 10): 8
  ...
  eye voxels span x [3, 4, 7, 8] z [15, 16]; every eye voxel is a surface colour (not a carved hole): True
exposed faces: 796  (budget 2000)
triangles    : 1592  (what the startup log reports)
faces per colorIndex: 1:48, 2:160, 3:32, 4:32, 5:54, 6:54, 9:4, 10:16, 11:94, 12:94, 13:18, 14:18, 17:74, 18:72, 19:26
...
rendered gamecam (perspective): 760x620 @ 16.0 px/voxel
gamecam camera: eye=(32.63, 38.59, 21.40) target=(6.00, 4.50, 9.00) dist=45.0 voxels (4.5 blocks) az=38.0 el=16.0 focal=720.0px
perspective self-check: a 1-voxel ground segment projects to 12.73 px at the near end vs 9.52 px at the far end (ratio 1.336; an orthographic camera would give 1.000)
...
| 眼格在占用表内 + `+y` 邻居为空 | 表面着色，不挖空 | 8 格全部 `True/True`（2×2 ×2 只眼，开在正面 y=7；`+y` 的 y=8 只在 z=17 的眉弓上有格） | ✓ |
...
| 3 | 朝向：PNG **第 0 行 = 面的上沿** | **不适用**（§9：生物模型的画布不是方块贴图；本条管的是 16×16 方块三面） | 不适用 |
| 4 | alpha：不透明全 255 | ✓ `{255}` | ✓ `{255}` |
| 5 | 色板：每个 RGB 都在 §2.1 的 32 色表内 | ✓ 15 个索引全在表内（表见 §5.1 回读段） | ✓ 15 个索引全在表内（§5.3） |
| 6 | 低饱和：每色 S ≤ 0.50 | 最高 **0.471**（F2） | 最高 **0.414**（E0） |
| 7 | 构图：无 4 px 以上实心块/无规则形/无抗锯齿 | **不适用**（同上：管的是方块贴图内部构图；体素模型只有平色块） | 不适用 |
...
> **§1/§3/§5/§6/§8 的适用性口径**已写在 `docs/art/01-style-guide.md` §9 的「生物模型贴图」条里
> （§2 色板 / §3 低饱和 / §6 alpha / §8 的 1·2·4·5·6·9·10 项适用；§1 与 §5 只约束方块贴图）。
...
- **调色板一字未动**（Mossback）：重建出的 `assets/palettes/mossback.png`
  md5 `74dcc2a82723523d9f1d0b40b92ba7db` 与仓库原文件**逐字节相同** ——
  返工只动形体，没顺手改色。
...
前后各出 ~0.36 格，**同一量级、同一做法**（卡面 §1.2 的口径澄清成立：对齐的只是比例，缩放口径不变）。

**外观身份保留清单**（逐条对卡面 §1.2.6 "一律不变"）：

| 特征 | 是否保留 |
|---|---|
| 色板 | ✓ 调色板 PNG 逐字节相同；体素用到的**用色集合与 v4 逐索引相同**（v4 = `1 2 3 4 5 6 7 17 18 19 20 21 22 23 24`，v5 = 同 15 个；`per-colorIndex` 表可比） |
| 关节标注 1..7 | ✓ 躯干/头/左右前肢/左右后肢/尾 = 索引 1..7；9..16 仍未用（`second-colour indices painted: []`） |
| z=0 层只许腿 | ✓ 只剩索引 3/4/5/6 |
| 正面 +Y | ✓ 鼻端在 y 最大侧；侧视图一律"鼻在右" |
| 苔丘（识别特征） | ✓ 保留，但按卡面压成 **1 格薄盖**（卡面 §1.2.2） |
...
| 四肢 ≥ 2 格宽 | 臂 x1..2/x9..10（2 宽 × 3 深）；腿 x4..5/x6..7（2 宽 × 3~4 深） | `per-colorIndex` 里 idx 3/4/5/6/11/12/13/14 的 x/y 跨度即可核 |
| 肘 / 膝弯折 | 肘：上臂 y1..3 → 肘 y2..5 → 前臂 y5..7（往前折）；膝：大腿 y1..3 → 膝 z=3（y1..4 加厚）→ 小腿 y2..4（前移 1 格） | 侧视出图可见 |
...
| 头有骨相：眉弓 | z=17 的颅骨多伸一格到 y=8（眼面在 y=7）⇒ 眉弓比眼面凸 1 格 | idx 2 的 y 上界 8 > 眼的 7 |
| 头有骨相：下坠下颌 | z=13 的下颌 x4..7 × y6..8（比颅窄 2 格、探到 y=8） | z=13 层可见 |
| 眼窝 2×2（仍 = 索引 10，不挖空） | 正面两角 x3..4 / x7..8 × z15..16 | **8 格**（2 只眼 × 2×2） |
| 契约 v2 用法照旧 | 1..8 主标签、9..16 第二色、17+ 躯干组 | 见 §5.3 的第二色落盒表 |

**v2 第二色本卡用了六个**（v4 是四个）：9 胸前空洞、10 眼窝、11/12 骨手（含前臂）、
**13/14 双脚（本卡新增用）**。用途栏的一句话理由：`13/14 = 关节 4/5 的第二色`，
把"骨架一路露到脚上"画出来（左/右脚各 8 格）；它们**不增加关节数**（仍然是 6）。
...
| Mossback v5b | 腹线 z=7 改成**整宽 x0..6**（只把 y 两端各收 1 格），头改成"颅 + 前额 + 吻"三级台阶 | 腿"从身体四角长出来" ✓；头不再是平板方块 ✓（`mossback-v5_contact_sheet.png`） |
| Mossback v5c | 背线 z=12 整层由受光色 '9' 改回主色 '1'（苔盖正压在它上面，侧视只露一条暗背线） | 头与苔都更跳；体素数 705 不变 |
| Mossback v5d | 下腹两侧竖边（z=8 的 x0/x6 两列）由泥色 'b' 改回暗色 'a'（W0） | 下腹有暗边、形体更立体；顺带把**用色集合补回与 v4 相同的 15 个索引**（此前漏用索引 18） |
| Wretch v5a | 四肢加粗到 2 格宽、画布加宽 | **正面剪影从肩到手仍是一整块**（臂 x1..2 与腿相邻）—— v1 的教训换了个位置复发 ✗ |
| Wretch v5b | 臂外移 + 腿收进躯干正下方（x4..7），留出 x=3/x=8 两条竖向空隙 | 正面读出「臂 \| 躯干+腿 \| 臂」三段 ✓ |
| Wretch v5c | 胸腔 4×4、锁骨下凹 2 格深、锯齿破布边、眉弓 + 下坠下颌、眼窝 1×1 → 2×2、双脚上骨色 | 定稿（`hollow_wretch-v5_contact_sheet.png` / `_gamecam.png`） |

## 10. 已知问题（含需 PM 裁决/知悉项）

### 10.1 ★ 需裁决：卡面 §1.2.2 的"苔最多中部 2 格"与验收判据互斥

卡面 §1.2.1 写死"腿 7 + 躯干 6 + 苔 1 = 14"，§4.2 又要求"z 跨度仍恰 14"；
而 §1.2.2 的括号里允许苔"最多中部 2 格"。**这两条同时满足时苔顶会到 z=14 ⇒ z 跨度 15**。
本卡按**验收判据**执行：苔只有 z=13 一层（`idx 20..24` 的 z 上界 = 13）。
若将来确实要"中部鼓起 2 格"，须先改高度预算（并接受 z 跨度 15 或把腿/躯干让出一格）。
...
这是卡面自己两条要求联立的结果，不是美术侧的自由发挥；若 PM 希望"头低于背线"，
需要改 §4.3 那条判据（或把苔改到背线以下）。

### 10.3 需 PM 知悉：Mossback 躯干宽取 7（卡面给的是"7~8"区间）

取 7 的唯一理由是**头要居中**：躯干 7 与头 5 同为奇数 ⇒ 头左右各让 1 格完全对称；
取 8 时 5 宽的头只能偏 0.5 格（0.05 格，肉眼几乎看不出，但"每索引包围盒"表会显形）。
副作用：腿粗/躯干宽 = 2/7 = 0.286（原版 4/12 = 0.333；取 8 时是 0.25）——7 更贴原版。
若 PM 更看重"卡面 §6.4 的 8"，改一个 `GRID_X` 与两层字符稿即可，比例账会随之变成 0.25。

### 10.4 需 PM 知悉：Wretch 的躯体比碰撞盒宽

实体占 x1..10 = **1.00 格宽**（与 v4 的 0.60 相比变宽），碰撞盒仍是 0.6 宽 ⇒
**手臂外缘探出碰撞盒 0.2 格**。卡面 §2.1 明说"手臂略探出碰撞盒 0.6 与原版人形一致"，
本稿的臂/盒关系与原版人形一致（原版模型 1.0 宽、盒 0.6）。**若 PM 理解为"必须 ≤0.6+0.6=1.2"**，
本稿 1.00 也在内；若是"必须恰好 0.6"，则需要把臂收回一格（会重新踩到 v5a 的坑）。

### 10.5 诚实登记：本轮**没有**做的取证

- **实机暂停帧 A/B**：卡面 §2.5 明确划给 PM 侧（"PM 侧另用暂停帧装置实拍复核"）。
  开发者侧只做了启动日志 + 渲染出图；**"游戏里它长这样"这一步尚未经过实机像素判据**。
- **MagicaVoxel（§4.6 可选加分）未开**：本机是否有 `opencraft_scratch/mv/` 需 PM 确认，按"未做"记。
- 渲染器是**自写的简易光栅器**（面明暗固定 6 档，非引擎光照）：它服务"形状可读性"，
  不能替代实机像素证据；两者结论冲突时以实机为准。

### 10.6 一处口径提示：Mossback 的"躯干最高点"

`head max z 13 vs all joint 0 (incl. moss/hump) max z 13` 是**取等号**：
躯干组的最高点就是苔盖（z=13）。契约自查表里"头应高于躯干"那条，本稿按
**躯干主体（索引 1/17/18/19）**成立（97 行：`max z 12 < 13`）。这与 v4 的处境相同
（v4 也是"苔是全身最高点"，只是当年苔顶 13 > 头 12）——本卡把它从"苔比头高"改成了"苔与头齐平"。

## 11. 给项目经理的备注

1. **两只模型各自独立可验** ✓：mossback 的判据全在 §5.1（z 跨度 / 腿高 7 / 四腿外缘逐格 / 苔顶≤头顶）；
   wretch 的全在 §5.3（≥450 / z 跨度 18 / z=0 只脚 / 眼格表面着色 / 第二色落盒）。
2. **色板通道**：Mossback 的调色板**逐字节未变**；Wretch 的调色板只有 13/14 两格变了
   （U0 → S3），如果 PM 侧有历史 md5 断言，只有后者会翻。
3. **本卡未碰任何代码/构建/测试**：`ctest` 459/459 与基线同数；启动日志的
   `atlas: 60/63` 也一字未变（方块贴图无关）。
4. **待 PM 侧补的证据**：暂停帧装置实拍（建议直接复用
   `docs/qa/T-B3-2026-09-18/pm_verify/run_scene_pre.sh` 的"暂停前俯仰"手法 ——
   人形 1.8 格高、四足兽 1.4 格高，两只都要拍头顶）。
5. **T-B4（Blastbud）**：本卡未创建、未触碰 `assets/mobs/blastbud*` ✓；
   若 B4 要沿用"游戏机位"门槛，`vox_inspect.py` 的 `render_perspective` 可以直接复用
   （相机三参数在文件顶部常量里）。
