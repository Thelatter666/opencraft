...
| 2026-09-18 | **§2 预判修正 + §3.2.2 更正（追认"副本迁移"）**：① §2"794 会翻转"是 PM 误判——实跑证明**存量断言零翻转**（9 在 v1/v2 都归躯干），风险是"全绿掩盖语义漂移"，区分度靠新用例（卡面已按此改正）；② §3.2.2 原文要求"就地改 T-B2 的 tools"，与 §4 白名单/§5.1 自相矛盾——**追认开发者取位：四件套拷入本卡证据目录再改**（就地改会破坏 T-B2 验收物的 md5 复现链；历史 QA 证据目录默认不可变）。裁决 `docs/tasks/T-B2b.ruling.md` §2 | T-B2b 报告 §1 末 / PM 实跑 | PM |
