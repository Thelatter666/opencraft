...
| 2026-09-18 | §3 表 Goxel 行更正（正文快照不改，此行为准）：**Goxel 0.15.1 现确认可用**——PM 独立复验（启动→按 PID 枚举到 1920×1004 窗口→截图渲染正常，`docs/qa/T-B2-2026-09-18/pm_goxel_verified.png`，md5 `ad6bb40d…`）。T-R3"进程起但无窗口"的**最可能成因已定位**：二进制名是**小写 `goxel`**（在 `Goxel.app` 内），按 `Goxel` 名取 PID 拿到的是空/错值，按错 PID 枚举窗口必然为空——PM 复核当天第一次也踩中同一坑（当时代码已改对）。⇒ 美术侧可放心把 Goxel 作为 MagicaVoxel 的替代路线 | 用户口述 + PM 本机复现 | PM |
| 2026-09-18 | **上一条再次更正（口径收敛，`T-B2.ruling.md` §4）**："可放心作为替代路线"说过头了——上一条的复验只证明了**进程起得来、渲染正常、画布是空的**（那张证据图现在看恰是不加载的证据）。开发者四条路全留痕（`docs/qa/T-B2-2026-09-18/goxel_third_party_attempt.md`）⇒ 本机 Goxel 0.15.1 **收不进也发不出 `.vox`**。"逐层字符稿 + 自写编码器"**转正为主路线**（本案已验证可复现）；B3 及以后的卡面不得写"用 Goxel 出 .vox"，除非先在真人环境手工走通导入/导出 | `docs/tasks/T-B2.report.md` §6 已知问题 1 | PM |
| 2026-09-18 | **验收通过并合入（merge `73a37ef`）**，四项裁决见 `docs/tasks/T-B2.ruling.md`：①"一关节一色"→**批准扩约**（1..16 第二色，引擎侧，另开 T-B2b，B3 之前完成）；②模型比碰撞盒长 0.25 格→**接受**（视觉/碰撞解耦，硌人再走校准卡）；③Goxel 口径→收敛（上一条）；④苔顶高于头、四腿同色→**接受**（设计意图）。报告自报的"上屏无直接证据"缺口由 PM 暂停帧装置补齐：复现性 0 px、模型 vs 两立方体 20,496 px、定稿目检头/四腿/苔丘（`docs/qa/T-B2-2026-09-18/pm_verify/`） | 验收全记录 `pm_verify/README.md` | PM |
