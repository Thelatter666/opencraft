...
| mobs/mossback.vox | 自制 | 手绘（逐层字符稿 + 自写 .vox 编码器）；未用 AI、未用图像/体素编辑软件 | 设计稿摘要：一头背部长苔的四足食草兽。9×14×14 体素（1 体素=0.1 格，视觉 0.90×1.40×1.40）；短粗四腿 + 桶身 + 前伸低头的头（含吻/双耳）+ 小尾；背部 z12..13 三层苔垫带悬檐与不规则边，苔色沿侧腹下爬两格。关节按调色板索引标注（契约 v2）：1..8 主标签 = 1 躯干 2 头 3/4 左·右前肢 5/6 左·右后肢 7 尾，9..16 = 关节第二色（本模型未用），躯干组色 17..24；配色取 docs/art/01-style-guide.md §2.1 的 32 色表（木 W0-W3 / 土 E1 / 植生 F0-F4）。 | 2026-09-18 | 逐格手写 14 层字符稿（docs/qa/T-B2-2026-09-18/tools/mossback_layers.txt，每层 14 行×9 格）；用自写编码器只做字符→字节翻译（同目录 vox_build.py），无几何运算/无对称展开/无填充；限定色板量化；无缩放/无抖动/无抗锯齿 |
| palettes/mossback.png | 自制 | 手绘（16×16 字符稿 + 自写 PNG 编码器）；未用 AI、未用图像编辑软件 | 设计稿摘要：Mossback 的 256 格调色板，格号 = colorIndex（第 0 格永不使用）。1..8 = 关节主标签格（1 躯干 W1 / 2 头 W3 / 3-6 四腿 W0 / 7 尾 F1 / 8 备用 S5），9..16 = 关节第二色（契约 v2，本模型未用，暂空填 U0），17..24 = 躯干组颜色槽（W2/W0/E1/F3/F1/F0/F2/F4），25..255 未使用槽填 U0。 | 2026-09-18 | 逐格手写 16×16 字符稿（docs/qa/T-B2-2026-09-18/tools/mossback_palette.txt）；自写 PNG 编码器（zlib + 结构拼装）；alpha 全 255；无缩放/无抖动/无抗锯齿 |
| mobs/hollow_wretch.vox | 自制 | 手绘（逐层字符稿 + 自写 .vox 编码器）；未用 AI、未用图像/体素编辑软件 | 设计稿摘要：人形近战猎手「空洞苦役 / 枯槁者」。6×6×18 体素（1 体素=0.1 格，视觉 0.60×0.60×1.80，与碰撞盒等高）；掐到 2 格宽的细躯干 + 4 格宽的肩背拱起（驼峰在背后）+ 前伸低垂的苍白颅骨（**眼窝 = 头关节第二色，索引 10**，开在正面两角），躯干正面锁骨下挖掉一格深、露出躯干第二色（索引 9）的**空洞**，两条 1 格宽的细长腿（0.9 格）与前折长臂（0.9 格，末端骨色手 = 索引 11/12）；关节按调色板索引（契约 v2）：1 躯干 2 头 3/4 左·右臂 5/6 左·右腿（无尾无备用，共 6 关节），9..16 关节第二色、17..19 躯干组；配色取 docs/art/01-style-guide.md §2.1 的 32 色表（石系 S0/S1/S2/S4/S5 + 眼窝 O0 + 一点土 E0）。头 z=17 高于躯干最高点 z=14（驼峰顶），z=0 层只有两只脚掌 | 2026-09-18 | 逐格手写 18 层字符稿（docs/qa/T-B3-2026-09-18/tools/hollow_wretch_layers.txt，每层 6 行×6 格）；用自写编码器只做字符→字节翻译（同目录 vox_build.py），无几何运算/无对称展开/无填充；限定色板量化；无缩放/无抖动/无抗锯齿 |
| palettes/hollow_wretch.png | 自制 | 手绘（16×16 字符稿 + 自写 PNG 编码器）；未用 AI、未用图像编辑软件 | 设计稿摘要：Hollow Wretch 的 256 格调色板，格号 = colorIndex（第 0 格永不使用）。1..8 = 关节主色（1 躯干 S1 / 2 头 S4 漂白骨色 / 3-6 双臂双腿 S0 / 7-8 未用），**9..16 = 关节第二色（契约 v2 的首个实例）：9 躯干二色 S5 = 胸前空洞、10 头二色 O0 = 眼窝、11/12 左右臂二色 S3 = 手**，13..16 未用，17..19 = 躯干组（S2/S0/E0），20..255 未使用槽填 U0 | 2026-09-18 | 逐格手写 16×16 字符稿（docs/qa/T-B3-2026-09-18/tools/hollow_wretch_palette.txt）；自写 PNG 编码器（zlib + 结构拼装）；alpha 全 255；无缩放/无抖动/无抗锯齿 |
