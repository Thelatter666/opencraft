...
2. ~~派发 T-D13~~ → ✅ **已合入（225/225）**。⚠ 但**用户需重新实机验收**：
   本卡消除的是**所有垂直突变**的瞬移感（潜行姿态 0.35 格：一帧跳 0.35 → 0.000063，
   约 23 帧收敛；跳跃弧线单帧 |ΔY| 0.1218 → 0.0865），**台阶场景实机仍不可达**（见 T-D17）。
3. **T-D14（Auto-Jump）待 T-D13 实机确认后再派**：它会让 1 格坎真的"跳"上去，
   但代价是滞空 0.6 s + 水平掉速约 21%（MC 原版行为）。用户需先确认能接受。
