...
| T-D11 | 冰/蓝冰/黏液方块摩擦内容接入（`BlockDef` 增 friction 并接线） | pending | act_mu1lx3d5_07f5ad5258d0 | ⚠ 冰/黏液稳态须**低于**默认方块（更滑 ≠ 更快） |
...
2. **派发 T-D8（自动上台阶 step height 0.6）**：T-D7 碰撞管线已重构完毕，台阶逻辑此时接入
   不再返工。玩家被 1 格高坎卡住、水平速度归零需手动跳，观感影响明显（T-D1 gap #5）。
3. T-D10（高速防穿透回归）**M2 载具卡前**必须做：船蓝冰 72.73 m/s = 3.64 b/tick，
   分析表明 `max_substep=0.5` 可覆盖（8 子步、每步 0.455 格）但**从未实测**。
4. T-D11/T-D12（冰面方块内容、水流体模型）为 M2 内容类，按 M2 排期取用。
5. 进入 M2（生存规则：库存/合成/饥饿/战斗/生物/刷怪），按 docs/05 逐卡拆分
