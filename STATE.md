...
## 备忘（有效临时项；收口时删除过期条目）
- **P2 待证（T009 视真人结果处理，勿盲目改码）**：ESC 暂停态下再次 ESC 未恢复——**仅在 System Events 合成键盘事件路径下观察到**（暂停方向同路径成功，证明事件确实到达 GLFW）；**物理键盘从未验证**（早前报告"真键盘不响应"系 PM 措辞错误，已更正）。**若真人按两次 ESC 能正常恢复 → 不是玩家可感知缺陷，T009 不应动代码**；若真人同样卡住 → 按缺陷修，怀疑点是暂停分支 `glfwSetInputMode(CURSOR_NORMAL)` 后按键边沿状态未正确轮询/清除。证据包 `docs/qa/p2-esc-2026-09-14/`（含"先真人验证再决定改码"的前置要求），真人结果须追加到该目录
...
- T009 卡面原文摘录（卡面本身不在仓库，由 PM 经用户转发，此处存档以便复核）：
  - spawn：「spawn 5×5 安全扫描（T004 备忘）本卡顺手做掉：出生点改为扫描地表最高实心方块。」
  - ⚠️ **流程缺口**：任务卡（接口契约的载体）不落仓库，复核者只能依赖 PM 转述。建议自 T010 起把卡面存档到 `docs/tasks/Txxx.md`（待用户确认后执行，含 T001–T009 回填）
