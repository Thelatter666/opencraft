...
| T-D14 | **Auto-Jump 机制（1 格坎自动跳，MC 1.10+）** | **已派发（2026-09-15，用户选定；卡面 `docs/tasks/T-D14.md`）** | act_mu2b9v46_66491e51c607 | ★ PM 实测修正：「水平掉速约 21%」**只对行走成立**，疾跑时基本不掉速，见下一步 3 |
...
3. ✅ **T-D14（Auto-Jump）已派发**（2026-09-15，用户在 T-D17 重定/Auto-Jump/M2 库存三选一中
   选定本卡）。MC 1.10+ 机制：前向扫描 + 注入跳跃冲量 + 真滞空，让 1 格坎真的"跳"上去。
   依据 `docs/research/08` §2–3、§5。
