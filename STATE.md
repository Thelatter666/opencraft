...
| T-D13 | 摄像机垂直弹簧阻尼（消除跨步瞬移感） | pending（卡面已建，基线 203/203） | act_mu1rsuc8_b511a2558449 | 物理层零改动；ζ=1.0 临界阻尼；过渡 0.15–0.25 s |
...
2. **派发 T-D13（摄像机垂直弹簧阻尼）**：用户 2026-09-15 实机反馈"上台阶像瞬移"。
   卡面已建、基线 **203/203**（T-D8 已合入）。物理层零改动，只把渲染相机 Y 用
   临界阻尼弹簧（ζ=1.0）与物理 Y 解耦。
